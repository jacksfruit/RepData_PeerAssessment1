...
```{r barplot_steps_per_day, echo=TRUE, fig.height=5, fig.width=20}
library(data.table)
library(ggplot2)
...
```
The total number of steps taken per day have a mean of `r format(mean, scientific=FALSE)` and a median of  `r format(median, scientific=FALSE)`.


## What is the average daily activity pattern?
Time series plot of the 5-minute interval and the average number of steps taken, averaged across all days:
```{r lineplot_avg_steps_per_interval, echo=TRUE, fig.height=10, fig.width=25}
...
```
`r format(max_interval, scientific=FALSE)` is the interval with the maximum average of steps across all days, which is `r format(max, scientific=FALSE)`.
...
```
There are `r format(cnt_obs_na, scientific=FALSE)` records in the data set where the value of the steps variable is missing (i.e. 'NA').
...
```{r barplot_steps_per_day_imputed, results='hide', fig.height=5, fig.width=20}
...
```{r mean_and_median_per_day_imputed, echo=TRUE}
mean_imp <- mean(dt_steps.per.day.imp$Total_Steps, na.rm = TRUE)
median_imp <- median(dt_steps.per.day.imp$Total_Steps, na.rm = TRUE)
```

The total number of steps taken per day - using the imputed data set - have a mean of `r format(mean_imp, scientific=FALSE)` and a median of  `r format(median_imp, scientific=FALSE)`.
...
```{r lineplot_avg_steps_per_interval_facets, echo=TRUE, fig.height=15, fig.width=25}
